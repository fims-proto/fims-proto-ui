...
const menuItems = computed(() => {
  ...
  const items: MenuItem[] = [
    // SoB selection
    {
      label: `${sobText.value} - ${periodText.value}`,
      items: [
        {
...
<template>
  <Menubar :model="menuItems">
    <template #start>
      <RouterLink :to="{ name: 'home' }" class="mr-3 text-xl font-extrabold">Fims</RouterLink>
    </template>
    ...
    <template #end>
      <div class="flex gap-2">
        <Button aria-label="Toggle Dark Mode" icon="pi pi-moon" @click="toggleDarkMode" />
        <Button
          type="button"

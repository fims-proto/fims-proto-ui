...
export default defineComponent({
  inheritAttrs: false,
  props: {
    type: String as PropType<'error' | 'info' | 'success' | 'warning' | undefined>,
    message: String,
    showIcon: {
      type: Boolean,
      default: true
...
<template>
  <div
    ...
    :class="{
      ...
      'bg-primary-50 text-primary-900': is('info') || is(undefined),
      'pl-4': !showIcon
    }"
  >
    <span v-if="showIcon" class="h-6 w-auto p-1">
      ...
      <information-circle-solid-icon
        v-if="is('info') || is(undefined)"
        class="h-4 w-4"
        aria-hidden="true"
      />
    </span>
